...
- Fonts: Space Grotesk for headings/body, JetBrains Mono for meta text (dates, tags, section labels, nav-adjacent details) — loaded via `next/font/google` in `app/layout.tsx`.
- Terminal-inspired touches: section headers are comment-style and mono (`// about`, `// experience`, `// projects`) instead of plain uppercase labels. A blinking cursor sits next to the name in the header.
...
- Dependencies: `framer-motion` is an approved dependency for the motion described above, and Space Grotesk/JetBrains Mono are approved font choices — no need to re-ask before using them. Beyond that, keep dependencies minimal — no carousel libraries, no additional animation/UI libraries unless explicitly requested.
